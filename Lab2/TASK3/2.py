...
ssc.socketTextStream("localhost", 9000).flatMap(lambda line: line.split(" ")).map(lambda word: (word.lower(), 1)).reduceByKey(lambda x, y: x + y).map(lambda rec: Tweet(rec[0], rec[1])).pprint()
